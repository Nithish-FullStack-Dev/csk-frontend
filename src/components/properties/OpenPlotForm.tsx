...
export function OpenPlotForm({
  ...
  const defaultValues: Partial<OpenPlotFormValues> = openPlot
    ? {
        ...
        customerName: openPlot.customerName || "",
        customerContact: openPlot.customerContact || "",
        agentName: openPlot.agentName || "",
    ...
    : {
        ...
        agentName: "", // Default to empty string
  ...
  const handleSubmit = async (data: OpenPlotFormValues) => {
    // Console log to debug if handleSubmit is called (should be if validation passes)
    ...
    try {
      ...
      if (thumbnailFile) {
        ...
        console.log("Thumbnail uploaded. URL:", finalThumbnailUrl);
      }
      ...
      console.log("Sending main payload:", payload);
