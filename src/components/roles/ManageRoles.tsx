...
const ManageRoles = () => {
  ...
  const getAllUsers = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_URL}/api/user/getUsers`, {
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        // Update role
        await axios.patch(`${import.meta.env.VITE_URL}/api/role/${editingRoleId}`, {
          description,
          color: selectedColor,
        });
      } else {
        // Create or set metadata
        await axios.post(`${import.meta.env.VITE_URL}/api/role/updateUserRole`, {
          name: roleName,
          description,
          color: selectedColor,
        });
  ...
  const handleClearRoleMeta = async (id) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_URL}/api/role/${id}/clear-meta`);
      fetchRoles();
    } catch (err) {
